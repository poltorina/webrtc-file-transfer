...
window.RTCMultiConnection = function(roomid, forceOptions) {
  ...
  function SocketConnection(connection, connectCallback) {
    ...
    connection.socket.on('cancel-file', function(fileId) {
      console.log('cancel-file', fileId);
      connection.onfilecancel(fileId);
    });
    ...
    connection.socket.on('userid-already-taken', function(useridAlreadyTaken, yourNewUserId) {
      ...
      connection.username = window.localStorage.getItem('myUsername') || connection.userid;
  ...
  function MultiPeers(connection) {
    ...
    this.addNegotiatedMessage = function(message, remoteUserId) {
      if (message.type && message.sdp) {
        ...
        if (connection.enableLogs) {
          // console.log('Remote peer\'s sdp:', message.sdp); // uncomment
        }
        return;
      }
      ...
      if (message.candidate) {
        ...
        if (connection.enableLogs) {
          // console.log('Remote peer\'s candidate pairs:', message.candidate); // uncomment
        }
        return;
      }
    ...
    this.onDataChannelClosed = function(event, remoteUserId) {
      ...
      event.username = window.localStorage.getItem('myUsername') || event.userid;
    ...
    this.onDataChannelError = function(error, remoteUserId) {
      ...
      error.username = window.localStorage.getItem('myUsername') || error.userid;
    ...
    this.onDataChannelOpened = function(channel, remoteUserId) {
      ...
      if (connection.peers[remoteUserId] && connection.peers[remoteUserId].channels.length) {
  ...
  function PeerInitiator(config) {
    ...
    // this.username = window.localStorage.getItem('myUsername') || this.userid;
    this.streams = [];
  ...
  var OnIceCandidateHandler = (function() {
    function processCandidates(connection, icePair) {
      ...
      // let isLocal = /(^127\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)|(^192\.168\.)/.test(candidate.split(' ')[4]);
      // if(isLocal) connection.peers[connection.userid].local = candidate.split(' ')[4];
  ...
  (function(connection) {
    ...
    connection.rejoin = function(connectionDescription) {
      ...
      console.warn("111111");
      var extra = {
        username: connection.username ? connection.username : connection.userid,
        browser: getBrowser()
      };
    ...
    connection.changeUserName = function(newUserName, callback) {
      callback = callback || function() {};
      connection.username = newUserName || window.localStorage.getItem('myUsername') || connection.userid;
      connection.socket.emit('changed-uuid', connection.username, callback);
    };
